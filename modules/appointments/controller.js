...
const getCustomerName = (firstName, lastName, customerFullName, fallback = "A customer") => {
  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        error: "Missing required fields (email, phone, service, time). Please ensure your profile has email and phone.",
      });
    }
    ...
    const finalStatus = (role === "owner" || role === "staff" || role === "admin") 
      ? "confirmed" 
      : "pending";
    ...
    try {
      ...
      const customerMessage = finalStatus === "confirmed"
    ...
    try {
      ...
      if (ownerId && finalStatus === "pending" && !isOwnerCreating) {
        ...
        const formattedDate = formatAppointmentDate(finalScheduledTime);
        const customerName = getCustomerName(firstName, lastName, customerFullName);
        ...
        console.log('Appointment created by owner/staff - no owner notification needed');
      } else if (isOwnerCreating) {
        console.log('Owner creating appointment - no owner notification needed');
      }
    } catch (ownerNotificationError) {
      // Log error but don't fail appointment creation
      console.error("Error creating owner notification:", ownerNotificationError);
    }
    ...
    if (finalStatus === "confirmed") {
      try {
        const emailHtml = `
          ...
          <p>Your appointment at <b>${getSalonName(salonInfo)}</b> is confirmed.</p>
          <ul>
            <li><b>Services:</b> ${serviceSummary}</li>
            <li><b>Time:</b> ${new Date(finalScheduledTime).toLocaleString()}</li>
    ...
    if (finalStatus === "confirmed") {
      try {
        const notificationSettings = await salonService.getSalonNotificationSettings(resolvedSalonId);
        ...
        if (reminderTime > new Date()) {
          const reminderMessage = `Reminder: You have an appointment at ${getSalonName(salonInfo)} on ${appointmentDate.toLocaleString()}`;
...
exports.getAppointmentById = async (req, res) => {
  try {
    ...
    if (role === "customer" && appointment.user_id !== (req.user?.user_id || req.user?.id))
...
exports.updateAppointment = async (req, res) => {
  try {
    ...
    console.log('Update appointment request:', { id, role, updates, userId: req.user?.user_id });
    ...
    if (role === "owner") {
      ...
      if (salonInfo && salonInfo.owner_id !== req.user?.user_id && salonInfo.owner_id !== req.user?.id) {
        return res.status(403).json({ error: "Forbidden: You don't own this salon" });
      }
    }
    ...
    const timeChanged = updates.hasOwnProperty("scheduledTime") || updates.hasOwnProperty("scheduled_time");
    const oldScheduledTime = appointment.scheduled_time;
    const newScheduledTime = updates.scheduledTime || updates.scheduled_time || oldScheduledTime;
    ...
    if (timeChanged && oldScheduledTime !== newScheduledTime) {
      try {
        ...
        const salonName = getSalonName(salonInfo);
        await notificationService.cancelScheduledReminders(appointment.user_id, salonName);
      } catch (reminderError) {
    ...
    if (timeChanged && oldScheduledTime !== newScheduledTime && updated.status !== "cancelled") {
      try {
        const notificationSettings = await salonService.getSalonNotificationSettings(appointment.salon_id);
